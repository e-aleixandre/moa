...
| `--preview-port` | `0` | Initial local port for the [Live Preview proxy](./serve.md#the-live-preview-proxy). With `0`, Moa suggests its own port plus one. The listener binds `127.0.0.1` on first use and closes when the preview does |
| `--preview-public-url` | | Legacy fallback for API clients that do not send their own address. The web UI derives the address per browser from Moa's host and the listener port; this flag is not required |
...
| `gpt-6` | `gpt-6-astra` |
| `grok` | `grok-4.7` (xAI) |
| `grok-4.7-build` | `grok-4.7` (the subscription backend's name for it) |
