...
// ways: the subscription backend answers "grok-4.7-build", the API backend
// "grok-4.7". Same model and same pricing, so both must land on one entry —
...
func TestBackendSpellingResolvesToPricing(t *testing.T) {
	m, ok := ResolveModel("grok-4.7-build")
	if !ok {
		t.Fatal("grok-4.7-build must resolve")
	}
	if m.Pricing == nil {
		t.Fatal("grok-4.7-build resolved without pricing")
	}
	canon, _ := ResolveModel("grok-4.7")
	if m.ID != canon.ID || m.Pricing.Input != canon.Pricing.Input {
		t.Errorf("grok-4.7-build = %s (input %.2f), want same as grok-4.7 (input %.2f)",
...
func TestSameModelIdentityAcrossBackends(t *testing.T) {
	if !SameModelIdentity("grok-4.7", "grok-4.7-build") {
