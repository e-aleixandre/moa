...
func TestGPT56TerraPricing(t *testing.T) {
	...
	p := model.Pricing
	if got, want := *p, (Pricing{Input: 2, Output: 12, CacheRead: 0.2, CacheWrite: 2.5, FastMultiplier: 2, Tiers: []PricingTier{{Threshold: 272_000, Input: 4, Output: 18, CacheRead: 0.4, CacheWrite: 5}}}); !reflect.DeepEqual(got, want) {
...
func TestGPT56LunaPricing(t *testing.T) {
	...
	p := model.Pricing
	if got, want := *p, (Pricing{Input: .2, Output: 1.2, CacheRead: .02, CacheWrite: .25, FastMultiplier: 2, Tiers: []PricingTier{{Threshold: 272_000, Input: .4, Output: 1.8, CacheRead: .04, CacheWrite: .5}}}); !reflect.DeepEqual(got, want) {
...
func TestThinkingAlwaysOn_PerModel(t *testing.T) {
	...
	for id, want := range cases {
		if got := ThinkingAlwaysOn(Model{ID: id}); got != want {
			t.Errorf("ThinkingAlwaysOn(%s) = %v, want %v", id, got, want)
		}
	}
}
