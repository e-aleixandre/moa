...
function decodeEnvelope(envelope) {
  const encoded = envelope.slice('moa-link-v1:'.length);
...
test('Pulse pairing envelope is a base64url JSON envelope', () => {
  const envelope = encodePulsePairingEnvelope('https://moa.example', 'moa-link-v1:id:secret');
  expect(envelope).toStartWith('moa-link-v1:');
  expect(envelope).not.toMatch(/[+/=]/);
  expect(decodeEnvelope(envelope)).toEqual({
    server_url: 'https://moa.example',
    pairing_payload: 'moa-link-v1:id:secret',
  });
});
...
test('Pulse pairing creation uses the owner request boundary without caching', async () => {
  ...
  const result = await createPulsePairing(async (path, options) => {
    ...
    return { ok: true, json: async () => ({ payload: 'moa-link-v1:id:secret' }) };
  });

  expect(result).toEqual({ payload: 'moa-link-v1:id:secret' });
