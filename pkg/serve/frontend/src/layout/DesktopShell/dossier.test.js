...
// diff: with Ambient off the desktop must stay the two-zone shell it has always
// been, the grid must NOT grow a dossier (several sessions, no single "this
// session"), and the dock threshold is a measurement — 264 + 844 + 340 — not a
// round number, so it is asserted against its parts.
...
test('with Ambient off there is no third zone at all', () => {
  expect(desktopDossierView({ ambient: false, view: 'conversation', session, panel: open })).toBe(null);
});

test('the grid has no dossier: "this session" has no single answer there', () => {
  expect(desktopDossierView({ ambient: true, view: 'grid', session, panel: open })).toBe(null);
});

test('no focused session, nothing to hold a dossier for', () => {
  expect(desktopDossierView({ ambient: true, view: 'conversation', session: null, panel: open })).toBe(null);
});

test('the conversation owns it, and carries the open page through', () => {
  expect(desktopDossierView({ ambient: true, view: 'conversation', session, panel: { open: true, page: 'usage' } }))
    .toEqual({ open: true, page: 'usage' });
});

test('a closed panel still mounts the zone: it slides, it does not appear', () => {
  expect(desktopDossierView({ ambient: true, view: 'conversation', session, panel: { open: false, page: 'root' } }))
