...
// name + what it's doing + mono time. It is CONNECTED: the container
...
//   • bash     → NO identity accent: neutral overlay1 spinner + mono
//                `bash` label; teal never doubles as "a bash job".
//
// Click a chip → onOpen(id) opens that subagent's SubagentView. The
// spinner reuses the Spinner primitive (color variants distinguish parallel
// subagents; overlay1 is the neutral bash color).
//
// TODO(sticky mirror): the tray should only render when the
...
// whenever there are live agents; the container gates it minimally.
