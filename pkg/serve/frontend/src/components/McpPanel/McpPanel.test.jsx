// McpPanel.test.jsx — a remote server waiting for sign-in: how it reads in the
// list, and the Connect → paste → Finish flow, driven through the panel's own
// handlers with fetch and window.open faked.
import { test, expect, mock, beforeEach, afterEach } from "bun:test";

// The panel is walked as a plain vnode tree (no DOM), so its hooks are stubbed:
// `pick` decides what each useState returns, in call order across the render
// (McpPanel: 0 data, 1 failed, 2 openName; ServerDossier: 3 busy, 4 confirming,
...
const { McpPanel } = await import("./McpPanel.jsx");
...
// Renders the panel with `server` expanded and the dossier's oauth state set.
function render(server, oauth = null) {
  ...
  return expand(McpPanel({ sessionId: "s1", mcpTick: 0 }));
}
...
test("a server waiting for sign-in reads as needing sign-in, not failed", () => {
  const tree = render(serverIn());
  const pill = all(tree, hasClass("mcp-pill"))[0];
  expect(text(pill)).toBe("sign-in needed");
  expect(pill.props.class).not.toContain("mcp-pill-bad");
  expect(all(tree, hasClass("mcp-row-error"))).toHaveLength(0);
...
test("Connect or Reconnect replaces Restart, following auth_action", () => {
  ...
  expect(button(tree, "Connect")).toBeDefined();
...
test("Finish sends the pasted address and refreshes the panel", async () => {
  fetchReply = (path) =>
...
test("a rejected paste shows the server's message inline and keeps the field", async () => {
  ...
  expect(text(all(shown, hasClass("mcp-oauth-error"))[0])).toBe(
