...
func TestWsEventFromBus_MCPChanged(t *testing.T) {
	ev, ok := wsEventFromBus(bus.MCPChanged{
		SessionID: "s1", Total: 3, Ready: 1, Disabled: 1, Unhealthy: 1, Pending: 1, AuthRequired: 1,
	})
	...
	want := MCPChangeData{Total: 3, Ready: 1, Disabled: 1, Unhealthy: 1, Pending: 1, AuthRequired: 1}
	if data != want {
		t.Fatalf("Data = %+v, want %+v", data, want)
	}
}
