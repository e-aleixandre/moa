...
func TestNormalizeThinking_UsesLowForInternalNoReasoningCalls(t *testing.T) {
	for _, level := range []string{"", "off", "low", "medium", "high"} {
		got, err := normalizeThinking(level)
		if err != nil {
			t.Fatalf("normalizeThinking(%q): %v", level, err)
		}
		want := level
		if want == "" || want == "off" {
			want = "low"
		...
		if got != want {
			t.Errorf("normalizeThinking(%q) = %q, want %q", level, got, want)
		}
	}
	if _, err := normalizeThinking("xhigh"); err == nil {
		t.Fatal("normalizeThinking(xhigh) succeeded")
	}
}
