...
func TestStream_OAuthOmitsCacheBreakpoints(t *testing.T) {
	var raw []byte
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		var err error
		raw, err = io.ReadAll(r.Body)
	...
	ch, err := prov.Stream(context.Background(), core.Request{
		Model:    core.Model{ID: "gpt-5.6-luna"},
		System:   "stable instructions",
		Messages: []core.Message{core.NewUserMessage("hello")},
	})
	if err != nil {
	...
	if strings.Contains(string(raw), "prompt_cache_breakpoint") || strings.Contains(string(raw), "prompt_cache_options") {
		t.Fatalf("OAuth body leaked explicit cache fields: %s", raw)
	}
	var body map[string]any
	if err := json.Unmarshal(raw, &body); err != nil {
		t.Fatal(err)
	...
	if body["instructions"] != "stable instructions" {
		t.Fatalf("instructions = %v, want top-level on the Codex backend", body["instructions"])
	}
}
