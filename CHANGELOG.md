...
- Anthropic **Claude Opus 5.5** is available as `opus`, with its 1M-token
  context window, 128K-token output limit, documented input/output/cache
  pricing, and compatibility with Claude Max OAuth.
...
- Images returned by `read` and MCP tools now reach OpenAI, xAI, and Meta
  models instead of being silently discarded from tool results. Mixed text
  and image results preserve their order; text-only results retain their
  existing wire format and cache breakpoints. Attachment storage is unchanged.
