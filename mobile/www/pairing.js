...
export function decodeEnvelope(text) {
  ...
  const quotes = { '"': '"', "'": "'", "“": "”", "‘": "’" };
  const closingQuote = quotes[normalized[0]];
  if (closingQuote && normalized.endsWith(closingQuote)) {
    normalized = normalized.slice(1, -1).trim();
  }
  // Clipboard managers and line wrapping can put whitespace inside a long
  // code. Whitespace is not part of this envelope, so removing it is
  // unambiguous, including when a wrap lands in the prefix.
  normalized = normalized.replace(/\s/g, "");
  if (!normalized.startsWith(ENVELOPE_PREFIX)) return null;
